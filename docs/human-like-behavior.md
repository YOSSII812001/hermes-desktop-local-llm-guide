# Hermes Agent Desktop を人間らしくする設定メモ（記憶・ゆらぎ・調子・文体・内面）

このメモは、ローカルLLM（12B）の個人メンター秘書を「もう一段だけ人間らしく」するための設定集です。
README.mdの第18章（cronの黙る判断）を前提に、その上へ5本柱を足しています。

別のAIや未来の自分へ渡すときは、README → 各詳細メモ → このメモ、の順で読ませてください。

## 全体像：5本柱と設計思想

足したのは次の5本柱と、1つの付随修復です。
...
- 柱5: 内面状態（感情・気分・バイオリズム・意識を決定論で計算し、文面の色に使う）
...
人格設定（SOUL.md）には、すべての会話に共通する短い文体だけを足します。
Script Outputの扱いは特定作業の手順なので、秘書スキル（SKILL.md）へ置きます。
役割の分け方は[コンテキスト設計ガイド](context-files-memory-skills.md)を確認してください。

ここはローカルモデルへの英語指示の方が安定したため、英語のまま載せます。
...
## 柱5: 内面状態（感情・気分・バイオリズム・意識）

### ねらい

柱3の「調子」は疲労トレンドでトーンを下げるだけの、後ろ向きの調整でした。
柱5はそこへ「今日のHermesはどんな心持ちか」という前向きの内面を一枚足します。
毎日・時間帯ごとに必然的に変わる内面を文面の色に使い、12Bのテンプレ感をさらに崩します。

### 仕組み

内面は4つの決定論的な部品でできています。乱数は感情の1か所だけ、それも seed 付きです。

- バイオリズム: 固定の起点日からの正弦波3サイクル（身体23日・感情28日・知性33日）＋時刻帯（サーカディアン）。乱数を使わず、毎日違う値になります。
- 気分（mood）: バイオリズムの感情サイクルと `mood_state.json` 直近3日の進捗・疲労を合成した valence（心地よさ）/ energy（活力）と、その象限から決まるラベル（凪いだ／前向き／少し気が急く 等）。
- 感情（emotion）: 気分で重み付けした5つのレジスタ（warm / calm / crisp / wistful / bright）から1つを選ぶ、唯一の乱数部品。seed は「日×時刻」由来なので、同じ時間に2回叩けば再現し、日や時間帯が変われば必ず変わります。グローバル乱数は汚しません。
- 今向いている意識（focus）: 15分heartbeatが会話のトピックを追い、`cron\focus_state.json` に current_focus・focus_since・depth・lingering_loop を残します。同じ話題が続けば depth が増え、弱い既定トピック（「今日の作業」）では深まりません。切替は30分のデバウンス付きです。

チェックイン側 `daily_conversation_context.py` は、これらを `## いまの内面` セクションへまとめて注入します。
数値は一切出さず、トーンの指示と、focus が深いときの「いま意識が向いていること」を自然文で渡すだけです。
書き出しヒントも内面のレジスタに合わせて選ぶようにし、6種から12種へ広げました。

### 見せ方のノブ（env）

内面をどこまで言葉にするかは env で切り替えます。cron は引数を渡せないので env が唯一のノブです。

- `HERMES_INNER_EXPRESSION=expressive`（既定）: Hermes は内面を上品な敬語で一文だけ述べます（例「今朝は心が凪いでおります」）。芝居がからず、装飾過多や自己卑下は避けます。SOUL / SKILL と system_prompt の上品さが常に優先です。
- `HERMES_INNER_EXPRESSION=implicit`: 内面は言葉にせず、トーンと語彙にだけにじませます。
- `HERMES_INNER_STATE=off`: 第5柱の注入を全停止し、旧4柱の挙動へ即戻します（安全弁）。
- `HERMES_BIORHYTHM_EPOCH`（既定 2026-06-04）/ `HERMES_EMOTION_SEED`（E2E再現用）/ `HERMES_FOCUS_SWITCH_MINUTES`（既定30）。

### 設計上の約束

- 状態は全てルールベースで、llama-server が落ちていても計算できます（柱の大原則）。LLM は色付けだけ。
- focus 追跡は別ファイル（`focus_state.json`）へ書くだけで、`autonomous_trigger_evaluator.py` の attention_score・should_notify・各しきい値には一切触れません。既存スコアリングは無干渉です（`--no-state-write` で score 不変を機械確認できます）。
- `emotion_sample()` は `random.Random(seed)` のローカルインスタンスを使い、既存 `random.choice` のグローバル状態を汚しません。
- SKILL.md には固定テンプレと「テンプレ禁止」が同居して定型を誘発していたため、テンプレを「反面教師（Anti-pattern）」として明示し直しました。
...
### cron\focus_state.json

```json
{
  "current_focus": "Hermes autonomy",
  "focus_since": "2026-06-08T09:30:00+09:00",
  "last_switch": "2026-06-08T09:30:00+09:00",
  "depth": 4,
  "previous_focus": "Codex pipeline",
  "lingering_loop": "請求書テンプレの確認",
  "history": [
    { "topic": "Codex pipeline", "from": "2026-06-08T08:00:00+09:00", "to": "2026-06-08T09:30:00+09:00", "depth": 3 }
  ],
  "updated_at": "2026-06-08T11:00:44+09:00"
}
```
